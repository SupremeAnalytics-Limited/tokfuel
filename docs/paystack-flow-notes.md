...
## Fee and webhook clarification

Paystack applies any customer-facing processing fee during checkout. TokFuel now sends only the service amount and displays a notice that Paystack may add its fee; TokFuel does not add a second fee. Source: https://paystack.com/pricing

TokFuel exposes `POST /api/paystack/webhook`, validates `x-paystack-signature`, and uses the webhook as the reliable server-side confirmation path. The app callback remains useful for immediate feedback, but a deployed HTTPS webhook URL is needed for dependable asynchronous confirmation if the user closes checkout before returning to the app.
