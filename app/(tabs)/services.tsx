...
export default function ServicesScreen() {
  ...
  return <ScreenContainer className="px-5" containerClassName="bg-background"><ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
    <Text style={styles.kicker}>TOKFUEL GIFTS</Text><Text style={styles.title}>{category ?? "Pick a way to show love"}</Text><Text style={styles.subtitle}>{category ? `Choose from ${category.toLowerCase()} gifts only.` : "Browse by gift type, then choose a quantity from 500 units upward. Every option shows your final price before payment."}</Text>
