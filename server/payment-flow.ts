...
export async function initializePurchase(input: { email: string; giftId: string; link: string; quantity: number; callbackUrl: string }) {
  ...
  // Paystack owns fee calculation. TokFuel sends only the service amount;
  // Paystack determines and displays any customer-side processing fee.
  const customerAmount = serviceAmount;
  ...
  try {
    ...
    return { ...payment, serviceAmount, customerAmount, currency: "NGN" as const };
  } catch (error) {
